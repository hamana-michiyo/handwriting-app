...
### ✅ 機械学習用データ生成バグ修正完了（v0.9.0） - ファイル名統一・検出ロジック統合・常時保存実装

**v0.9.0 機械学習用データ生成バグ修正**:
- 🐛 **バグ原因**: ファイル名不整合により`debug/ml_char_*.jpg`が生成されず、Supabase保存時にフォールバック発生
- 🔧 **修正内容**: ファイル保存名と読み込みパス統一、文字セル検出ロジック完全統一
- 📊 **解決効果**: 機械学習用補助線除去済み画像が確実に生成・保存、フォールバック回避
- ⚡ **処理最適化**: 重要データ（Gemini用・機械学習用）はdebugモード無関係で常時保存

**v0.9.0 技術的修正**:
- ✅ **ファイル名統一**: `ml_training_{name}.jpg` → `{name}.jpg`に修正
- ✅ **文字セル検出統一**: `detect_char_cells`と`extract_right_character_regions_for_ml`を完全同一ロジックに統一
- ✅ **常時保存対応**: `_save_debug_image()` → `cv2.imwrite()`に変更（Gemini用・機械学習用）
- ✅ **検出条件改善**: 二値化パラメータ・面積計算・アスペクト比判定の完全統一

**v0.9.0 修正ファイル**:
- `improved_ocr_processor.py`: 文字セル検出統一、常時保存実装
- `supabase_ocr_processor.py`: ファイル読み込みパス修正

### ✅ 機械学習用データ生成バグ修正完了（v0.8.9） - 画像ファイル生成・保存処理改善

**v0.8.9 機械学習用データ生成バグ修正**:
- 🐛 **バグ原因**: ファイル名不整合により`debug/ml_char_*.jpg`が生成されず、Supabase保存時にフォールバック発生
- 🔧 **修正内容**: ファイル保存名と読み込みパス統一、文字セル検出ロジック完全統一
- 📊 **解決効果**: 機械学習用補助線除去済み画像が確実に生成・保存、フォールバック回避
- ⚡ **処理最適化**: 重要データ（Gemini用・機械学習用）はdebugモード無関係で常時保存
